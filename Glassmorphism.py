class BackDrop(QtWidgets.QGraphicsEffect):
    def __init__(self, blur: int = 0, radius: int = 0, backgrounds: list[dict] = None):
        ...
        self._animation = QtCore.QPropertyAnimation(self, b"animation_position")
    ...
    @staticmethod
    def _get_colored_pixmap(brush_color, pen_color, pen_width, width, height, radius):
        ...
        painter.drawRoundedRect(QtCore.QRectF(0.0, 0.0, width, height), radius, radius)
        painter.end()
    ...
    def _get_blur_background(self, source):
        ...
        background_expanded = source.copy(x * scale - self._blur, y * scale - self._blur, w * scale + self._blur * 2, h * scale + self._blur * 2)

        blurred_background_expanded = self._blur_pixmap(background_expanded, self._blur)

        blurred_background = blurred_background_expanded.copy(self._blur / 2, self._blur / 2, w, h)
    ...
    def _create_animation_pixmap(self, angle: int, line_width: int, color: QtGui.QColor):

        height = self._size.height()
        diagonal = height / math.sin(math.radians(angle))
        width = math.sqrt(diagonal ** 2 - height ** 2)
        offset = math.sqrt(line_width ** 2 + line_width ** 2)
        ...
        self._animation_pixmap = QtGui.QPixmap(width * 2 + offset * 2, height * 2)
        self._animation_pixmap.fill(QtCore.Qt.transparent)
    ...
    def draw(self, painter):
        ...
        mask = self._get_colored_pixmap(QtGui.QColor("#FFFFFF"), QtGui.QColor("#FFFFFF"), 1, w * scale, h * scale, self._radius * scale)

        self._cut_pixmap(main_background, mask, w, h)

        backgrounds = []
        for i, bg in enumerate(self._backgrounds):
            background = bg["opacity"], self._get_colored_pixmap(bg["background-color"], bg["border"], bg["border-width"] * scale, w * scale, h * scale, self._radius * scale)
        ...
        if self._animation.state() == QtCore.QPropertyAnimation.State.Running:
            ...
            pixmap_painter.setRenderHints(QtGui.QPainter.Antialiasing | QtGui.QPainter.SmoothPixmapTransform)
            pixmap_painter.drawPixmap(self._animation_position.x(), self._animation_position.y(), self._animation_pixmap.width() / 2, self._animation_pixmap.height() / 2, self._animation_pixmap)
            pixmap_painter.end()
    ...
    def _shine_animation(self, duration: int = 300, forward: bool = True, angle: int = 135, width: int = 40,
                  color: QtGui.QColor = QtGui.QColor(255, 255, 255, 125)):
        if self._animation.state() != QtCore.QPropertyAnimation.State.Running:
            self._create_animation_pixmap(angle, width, color)
            if forward:
                start_point = QtCore.QPointF(-self._animation_pixmap.width() / 2, 0)
        ...
        elif self._animation.state() == QtCore.QPropertyAnimation.State.Running and not self._forward_animation:
            self._forward_animation = False
            ...
            self._animation.setEndValue(end_point)
            self._animation.setDuration(duration - self._animation.currentTime())
            self._animation.start()

        elif self._animation.state() == QtCore.QPropertyAnimation.State.Running and self._forward_animation:
            self._forward_animation = False
...
class BackDropWrapper(QtWidgets.QWidget):
    def __init__(self, widget, blur: int = 0, radius: int = 0, backgrounds: list[dict] = None, shine_animation: list = None, move_animation: tuple = None):
    ...
    def enable_shine_animation(self, duration: int = 300, forward: bool = True, angle: int = 135, width: int = 40,
                        color: QtGui.QColor = QtGui.QColor(255, 255, 255, 125), shine_animation: tuple = None):
        if shine_animation is None:
            self._shine_animation_info = (duration, forward, angle, width, color)
        else:
            self._shine_animation_info = shine_animation

    def enable_move_animation(self, duration: int = 300, offset: tuple = (0, 0), forward: bool = True, move_animation: tuple = None):
    ...
    def event(self, event: QtCore.QEvent) -> bool:
        if event.type() == QtGui.QHideEvent.Type.HoverEnter:
            if self._shine_animation_info is not None:
                duration, forward, angle, width, color = self._shine_animation_info
                self.boxShadow._shine_animation(duration, forward, angle, width, color)
        ...
        if event.type() == QtGui.QHideEvent.Type.HoverLeave:
            if self._shine_animation_info is not None:
                duration, forward, angle, width, color = self._shine_animation_info
                self.boxShadow._shine_animation(duration, not forward, angle, width, color)
    ...
    def _move_animation(self, duration: int = 300, offset: tuple = (0, 0), forward: bool = True):
        ...
        if self._animation.state() != QtCore.QPropertyAnimation.State.Running:
            if forward:
                self._normal_pos = self.property("pos")
                self._forward_animation = True
                end_pos = QtCore.QPointF(self.property("pos").x() + offset[0], self.property("pos").y() + offset[1])
            else:
                self._forward_animation = False
            ...
            self._forward_animation = True
            end_pos = QtCore.QPointF(self._normal_pos.x() + offset[0], self._normal_pos.y() + offset[1])
